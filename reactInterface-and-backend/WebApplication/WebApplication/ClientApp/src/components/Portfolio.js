...
import { TradingViewChartWrapper } from './TradingView';
import FormControl from '@material-ui/core/FormControl';
import Button from '@material-ui/core/Button';
import { PortfolioTableWrapper } from './PTable';
import { PortfolioStatWrapper } from './PStats';
...
const Portfolio = () => {
    ...
    const [portfid, setPortfid] = React.useState('Please Select a Portfolio');

    const [selectedP, setSelectedP] = React.useState(0);

    // Portfolio Statistics
    const [cpLength, setCPLength] = React.useState(0);
    const [cpValue, setCPValue] = React.useState(0);
    const [cpPLDollar, setPLDollar] = React.useState(0); // Profit or Loss 
    const [cpPLPercent, setPLPercent] = React.useState(0);
    const [cpDrawdown, setCPDrawdown] = React.useState(0);
    const [cpTrades, setCPTrades] = React.useState(0);




    // When user requests for a portfolio to be purchased
    const handleSubmit = event => { 
        alert("User selected: " + portfid);
        console.log(TableData[portfid]);
        Api.buyPortfolio(TableData[portfid]);
    ...
    useEffect(() => {
        //fake api call to get the chart data
        setChartData(fakePortfolioHistoricalData);
    }, []);

    // API call to set Portfolio data for the table to use
    const [TableData, setTableData] = useState([]); 
    useEffect(() => {
        let getData = async () => {
            var data = await Api.getPortfolioTable(); // Api call to get portfolio data from DB
            if (data) {
                setTableData(data);
            };
        }
    ...
    // Set inital size of the table and chart components
    const [chartSize, setChartSize] = useState({ w: 0, h: 0 });
    const [tableSize, setTableSize] = useState({ w: 0, h: 0 });

    // Resize chart and table when needed
    useEffect(() => {
        updateChartSize();
        updateTableSize();
    ...
    const updateChartSize = () => {
        let _divSize = document.getElementById("chart-holder");
        if (_divSize) {
            const { width, height } = _divSize.getBoundingClientRect();
            //console.log(width, height);
            setChartSize({ w: width - 160, h: height - 160 });
        }
    }
    const updateTableSize = () => {
        let _divSize = document.getElementById("table-holder");
        if (_divSize) {
            const { width, height } = _divSize.getBoundingClientRect();
            //console.log(width, height);
            setTableSize({ w: width - 160, h: height - 160 });
        }
    }
    return (
        ...
        <div className={classes.root}>
            <Grid container spacing={1}>
                <Grid item xs={12}>
                    <Paper className={classes.profile_sum}>
                        <Typography component="div">
                            <Box fontWeight="fontWeightBold" m={1} /* Profile Summary Component */ >
                                Profile Summary:
                            </Box>
                        </Typography>
                    </Paper>
                ...
                <Grid item xs={6}>
                    <Paper className={classes.component} id="chart-holder" style={{ height: '623px' }} /* Chart Component */ >
                        <TradingViewChartWrapper data={chartData} size={chartSize} />
                    </Paper>
                </Grid>
                <Grid item xs={6}>
                    <Paper className={classes.component} id="table-holder" style={{ height: '623px' }} /* Table Component */ >
                        <PortfolioTableWrapper setPortfolioId={setPortfid} setCurrentPortfolioLength={setCPLength} tableData={TableData} />
                    </Paper>
                </Grid>
                <Grid item xs={6}>
                    <Paper className={classes.component} /* Portfolio Statistics Component */> 
                        <PortfolioStatWrapper PortfolioLength={cpLength} />
                    </Paper>
                </Grid>
                <Grid item xs={6}>
                    <Paper className={classes.component} /* Buy Portfolio Component */ >
                        <form onSubmit={handleSubmit}>
                            <div>
                                <FormControl className={classes.formControl}>
                                    <TextField id="standard-read-only-input"
                                        value={portfid}  // Set value to selected portfolio from the table
                                        InputProps={{
                                            readOnly: true,
                                        }}
                                    />
...
// Fake Charting Data
const fakePortfolioHistoricalData = [
